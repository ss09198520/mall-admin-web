...
const props = defineProps({
  ...
  function updateWithD3Data(d3Data) {
    state.nodes = state.nodesData.reduce((items, nodeData) => {
    ...
    state.relationships = state.relationsData.reduce((items, relationData) => {
      let sourceNode = state.nodesData.filter(function(n) {
          ...
          targetNode = state.nodesData.filter(function(n) {
  ...
  function updateNodesAndRelationships() {
    ...
    state.nodes = state.nodes.filter(function(d) {
      return d.collapsing === 0;
    });
    state.relationships = state.relationships.filter(function(d) {
...
function updateRelationships() {
  ...
  const nodeClick = (event , d) => {
    ...
    if (typeof state.options.onNodeClick === 'function') {
      state.options.onNodeClick(d);
    }
    updateNodesAndRelationships()
  }
