...
//                        'zoomOut': 'img/twemoji/1f50e.svg'
  ...
  const { queryMovieTile ,queryPersonName ,filterStr} = toRefs(state)
...
const appendIconToNode = (node) => {
   ...
   * 用戶查询
   **/
  async function queryMovieByTitle() {
    state.loading = true
    const {nodes, links} = await graphMovieByTitle(state.queryMovieTile)
    state.nodesData = nodes
    state.relationsData = links
    state.loading = false
    updateWithNeo4jData()
   ...
   * 用戶查询
   **/
  async function queryMovieByPersonName() {
    state.loading = true
    const {nodes, links} = await graphMovieByPersonName(state.queryPersonName)
    state.nodesData = nodes
    state.relationsData = links
    state.loading = false
    updateWithNeo4jData()
